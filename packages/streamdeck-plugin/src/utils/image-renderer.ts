...
type DynamicSourceButtonOptions = {
  name: string;
  channelLabel: string;
  channelColor: string;
  isActive: boolean;
...
type DynamicStatusButtonOptions = {
  channelLabel: string;
  channelColor: string;
  sourceName: string;
  status: 'routed' | 'idle' | 'error';
};
...
export function renderSourceButtonDynamic(options: DynamicSourceButtonOptions): string {
  const { name, channelLabel, channelColor, isActive, thumbnailBase64 } = options;
  const activeRing = isActive ? `<rect x="2" y="2" width="140" height="140" rx="8" ry="8" fill="none" stroke="${ACTIVE_COLOR}" stroke-width="4"/>` : '';
  ...
  const displayName = escapeXml(truncate(name, 14));
  const displayLabel = escapeXml(truncate(channelLabel, 12));
  ...
  <rect width="144" height="144" rx="12" ry="12" fill="#1a1a1a"/>
  <rect x="4" y="4" width="136" height="136" rx="10" ry="10" fill="none" stroke="${channelColor}" stroke-width="3"/>
  ${activeRing}
  ${thumbnailContent}
  <rect x="0" y="102" width="144" height="42" rx="0" ry="0" fill="rgba(0,0,0,0.7)"/>
  <text x="72" y="110" font-family="Arial,sans-serif" font-size="9" fill="${channelColor}" text-anchor="middle" dominant-baseline="middle" font-weight="bold">${displayLabel}</text>
  <text x="72" y="130" font-family="Arial,sans-serif" font-size="13" fill="white" text-anchor="middle" dominant-baseline="middle" font-weight="bold">${displayName}</text>
</svg>`;
}
...
export function renderStatusButtonDynamic(options: DynamicStatusButtonOptions): string {
  const { channelLabel, channelColor, sourceName, status } = options;
  const statusColor =
    status === 'routed' ? ACTIVE_COLOR : status === 'error' ? '#F44336' : '#666';
  const displayName = escapeXml(truncate(sourceName, 14));
  const displayLabel = escapeXml(truncate(channelLabel, 12));
  ...
  <rect x="4" y="4" width="136" height="136" rx="10" ry="10" fill="none" stroke="${channelColor}" stroke-width="3"/>
  <circle cx="72" cy="36" r="8" fill="${statusColor}"/>
  <text x="72" y="70" font-family="Arial,sans-serif" font-size="11" fill="${channelColor}" text-anchor="middle" dominant-baseline="middle" font-weight="bold">${displayLabel}</text>
  <text x="72" y="95" font-family="Arial,sans-serif" font-size="14" fill="white" text-anchor="middle" dominant-baseline="middle" font-weight="bold">${displayName}</text>
  <text x="72" y="120" font-family="Arial,sans-serif" font-size="10" fill="#aaa" text-anchor="middle" dominant-baseline="middle">${status === 'routed' ? 'LIVE' : status === 'error' ? 'ERROR' : 'IDLE'}</text>
</svg>`;
}
